  ...
  <section ref="sectionEl" class="py-24 bg-brand-900">
    <div class="max-w-7xl mx-auto px-6">
      ...
      <div class="text-center mb-16" :class="visible ? 'animate-fade-up' : 'opacity-0'">
        <p class="text-gold-400 text-xs font-semibold tracking-[0.4em] uppercase mb-3">Company Profile</p>
      ...
      <div class="grid grid-cols-2 gap-20 items-center">
        ...
        <div :class="visible ? 'intro-left' : 'opacity-0'">
          ...
          <div class="inline-flex items-center gap-3 px-4 py-2 rounded-full mb-8 year-badge">
            <div class="w-2 h-2 rounded-full bg-gold-400 animate-pulse" />
            <span class="text-gold-400 text-sm font-semibold tracking-widest">创立于 2018 年</span>
          </div>

          <p class="text-neutral-300 leading-8 text-base mb-6">
            中达聚财是一家以<span class="text-white font-semibold">人力资源服务为主业</span>的综合性企业集团，致力于为各类客户提供一体化解决方案，实现人才与组织的双向赋能。
          </p>
          <p class="text-neutral-400 leading-8 text-sm mb-8">
        ...
        <div class="grid grid-cols-2 gap-5" :class="visible ? 'intro-right' : 'opacity-0'">
          <div v-for="(item, i) in metrics" :key="i" class="metric-card"
            ...
            <p class="text-neutral-300 text-sm font-medium">{{ item.label }}</p>
            <p class="text-neutral-500 text-xs mt-0.5">{{ item.sublabel }}</p>
          </div>
        </div>
